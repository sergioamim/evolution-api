...
type InstanceGuardDependencies = {
  cache: CacheService;
  configService: ConfigService;
  prismaRepository: Pick<PrismaRepository, 'instance'>;
  waMonitor: Pick<WAMonitoringService, 'waInstances'>;
};

export const createInstanceGuards = ({
  cache,
  configService,
  prismaRepository,
  waMonitor,
}: InstanceGuardDependencies) => {
  ...
  async function instanceExistsGuard(req: Request, _: Response, next: NextFunction) {
  ...
  async function instanceLoggedGuard(req: Request, _: Response, next: NextFunction) {
  ...
  return { instanceExistsGuard, instanceLoggedGuard };
};
